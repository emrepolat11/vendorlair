...
function UpgradeBanner({ onUpgrade }) {
  return (
    <div style={{ background:"linear-gradient(135deg,rgba(108,99,255,0.2),rgba(116,192,252,0.1))",
      ...
      <div>
        <div style={{ fontWeight:700, fontSize:15, color:"#F0EDE6", fontFamily:"'Cormorant Garamond',serif", marginBottom:4 }}>
    ...
    <div style={{ background:"#111118", border:"1px solid rgba(255,255,255,0.07)",
      ...
      <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontWeight:700, fontSize:17, color:"#F0EDE6", lineHeight:1.2 }}>
            {vendor.name}
          </div>
          {vendor.website && (
            <a href={vendor.website.startsWith("http") ? vendor.website : `https://${vendor.website}`}
              target="_blank" rel="noreferrer"
              style={{ fontSize:11, color:"rgba(240,237,230,0.35)", textDecoration:"none", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {(vendor.poc_name || vendor.poc_email || vendor.poc_phone) && (
        <div style={{ background:"rgba(255,255,255,0.03)", borderRadius:10, padding:"10px 12px",
          ...
          {vendor.poc_name && <div style={{ fontSize:13, color:"rgba(240,237,230,0.8)", fontWeight:500, fontFamily:"'DM Sans',sans-serif" }}>👤 {vendor.poc_name}</div>}
          {vendor.poc_email && <div style={{ fontSize:12, color:"#6C63FF", fontFamily:"'DM Sans',sans-serif" }}>✉️ {vendor.poc_email}</div>}
          {vendor.poc_phone && <div style={{ fontSize:12, color:"rgba(240,237,230,0.35)", fontFamily:"'DM Sans',sans-serif" }}>📞 {vendor.poc_phone}</div>}
        </div>
      )}
      ...
      <div style={{ background:"#111118", border:"1px solid rgba(255,255,255,0.08)",
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <button onClick={onClose} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.08)",
            color:"rgba(240,237,230,0.5)", width:32, height:32, borderRadius:"50%",
        ...
        <div style={{ background:"#111118", borderRadius:20, padding:28,
          ...
          {success && (
            <div style={{ background:"rgba(47,158,68,0.12)", border:"1px solid rgba(47,158,68,0.25)",
              borderRadius:10, padding:"12px 16px", color:"#69DB7C",
              fontSize:13, marginBottom:20 }}>✅ {success}</div>
          )}
          {error && (
            <div style={{ background:"rgba(250,82,82,0.12)", border:"1px solid rgba(250,82,82,0.25)",
              ...
              fontSize:13, marginBottom:20 }}>⚠️ {error}</div>
          )}
